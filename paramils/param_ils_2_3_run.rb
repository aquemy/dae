...
#	@param_ils_traj_file.print "#{$total_cputime}, #{val}, #{n}, #{@iteration}, #{@numFlip}, "
	...
	@param_ils_traj_file.puts @params.map{|x|full_state[x]}.join(", ") # output the current values
...
def betterWithoutAutomaticIncrease(new_state_int, old_state_int, haveToFinish=false)
	...
	#=== Otherwise, the one with less evaluations looks better.
...
# =========================================
def haveToStop()
	return true if $totalEvaluationCount>=@maxEvals
	return true if $total_cputime >= @tunerTimeout
	return true if @iteration > @maxIts
	return true if @algo =~ /saps/ and @allVisitedStates.size >2400
	#=== Also stop if we have already reached the minimum we can achieve
	return true if detail(@incumbent_state_int) == @maxDetail and @cachedResultScalars[$full_int_to_stripped_int[@incumbent_state_int]][detail(@incumbent_state_int)-1] < @optimum_solqual + @eps
...
#	puts ARGV.join(" --- ")
	...
	out.puts "\n======================================================================================================"
	out.puts "PARAM_ILS version 2.3.5, copyright Frank Hutter, 2004-2010."
	...
	if is_win
		...
		out.puts "======================================================================================================"
		out.puts "paramils -numRun 0 -scenariofile example_saps/scenario-Saps-SWGCP-sat-small-train-small-test.txt -validN 100"
		out.puts "paramils -numRun 0 -scenariofile example_saps/scenario-Saps-single-QWH-instance.txt -validN 100"
		out.puts "paramils -numRun 0 -scenariofile example_spear/scenario-Spear-SWGCP-sat-small-train-small-test.txt -validN 100"
		out.puts "paramils -numRun 0 -scenariofile example_cplex/scenario-Cplex-CATS-smalltrain-smalltest.txt -validN 100"
	end
	out.puts "======================================================================================================"
end
...
if @scenariofile
	scenarioparams = {"algo" => @algo, "run_obj"=>@run_obj, "overall_obj"=>@overall_obj, "cutoff_time"=>@cutoff_time, 
			...
			"execdir"=>$execdir, "outdir"=>@outdir, "paramfile"=>$param_file}
...
if @instance_seed_file == ""
	...
	2000.times{|i|
		...
		inst = File.expand_path(entries[0])
		rest = entries[1...entries.length]
...
if @test_instance_seed_file == ""
	...
	File.open(File.expand_path(@test_instance_seed_file)){|file| 
		...
		entries = line.strip.split
		seed = entries[0].to_i
		inst = File.expand_path(entries[1])
...
@out.puts "algo: #{@algo}"
@out.puts "tunerTimeout: #{@tunerTimeout}"
