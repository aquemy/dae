...
int main(int argc, char **argv)
{
  ...
  while( di !=NULL)
	...
	{
	  if(di->o[i]*minmax[i] > best[i]*minmax[i])
	    best[i] = di->o[i];
	  if(di->o[i]*minmax[i] < worst[i]*minmax[i])
	    worst[i] = di->o[i];
	}
      count++;
      di = di->next;
  ...
  fprintf(fp, "\n");  
  fprintf(fp, "utopia_bound ");
  for(i=0;i<nobjs;i++)
    fprintf(fp, "%.9e ", best[i]+myabs(best[i]-worst[i])*minmax[i]*phi);
  fprintf(fp, "\n");

  fprintf(fp, "nadir_bound ");
  for(i=0;i<nobjs;i++)
    fprintf(fp, "%.9e ", worst[i]-myabs(best[i]-worst[i])*minmax[i]*phi);
  fprintf(fp, "\n");
