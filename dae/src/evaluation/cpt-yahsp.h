...
public:
    ...
    unsigned int l_max() const 
    { 
        return _l_max; 
    }
        
    void l_max( unsigned int l ) 
    {
        _l_max = l; 
    }
...
protected:
    ...
    double fitness_unfeasible_too_long( )
    {
        ...
        return _fitness_penalty * (l_max() + 1.0) * l_max() * (double)_unknown_parameter * 2;
    }
    
    double fitness_unfeasible_intermediate( EOT& decompo )
    {
        return _fitness_penalty * (l_max() - (double)decompo.get_number_useful_goals() + 1.0) * l_max() * (double)_unknown_parameter;
    }
      
    double fitness_unfeasible_final( EOT & decompo )
    {
        return _fitness_penalty * (l_max() - (double)decompo.get_number_useful_goals() + 1.0);
    }
