...
#include "utils/eoSteadyMetriContinue.h"
#include <daex.h>
template <class Indi>
 ...
 */
template <class Indi>
eoContinue<Indi> & do_make_continue_daemoeo(eoParser& _parser, eoState& _state, eoEvalFuncCounter<Indi> & _eval, moeoArchive < Indi > & _archive)
{
  //////////// Stopping criterion ///////////////////
  ...
  eoValueParam<unsigned>& minGenParam = _parser.createParam(unsigned(0), "minGen", "Minimum number of generations",'g', "Stopping criterion");
    if (_parser.isItThere(steadyGenParam) && steadyGenParam.value() > 0)
      {
        eoSteadyMetriContinue<Indi> *steadyCont = new eoSteadyMetriContinue<Indi> (minGenParam.value(), steadyGenParam.value(),_archive,true, 1.1);
        // store
