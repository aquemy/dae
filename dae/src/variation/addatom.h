...
public:
    ...
    MutationChangeAddAtom( 
            ...
            unsigned int maxtry_search_candidate = 0, 
            unsigned int maxtry_search_mutex = 0
        ) :
        _proba_change(proba_change), /*_proba_add(proba_add), */ 
        _times(times), 
        _maxtry_search_candidate(maxtry_search_candidate), 
        _maxtry_search_mutex(maxtry_search_mutex)
    {}
	...
	//	std::cout << "DECOMPO=" << decompo << std::endl;
        ...
        for(unsigned int k=0; k < decompo.size(); ++k ) 
        {
            ...
            if( r < _proba_change ) 
            {
                ...
                {
                    eo::log << eo::xdebug << " cannot replace (all atoms are mutex)";
                }
#endif
                r = rng.uniform();
                if(r < _proba_change) 
                {
                    ...
                    {
                        eo::log << eo::xdebug << " cannot add (all atoms are mutex) ";
                    }
#endif
                } // if r < proba_change
#ifndef NDEBUG
...
#endif // __DAEX_MUTATION_ADDATOM_H__
