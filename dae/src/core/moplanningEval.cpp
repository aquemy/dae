...
#include "moplanningEval.h"


PlanningEvalMO::PlanningEvalMO (unsigned int l_max_ ,
...
PlanningEvalMO::~PlanningEvalMO(){};

void PlanningEvalMO::operator()(PlanningMO & _decompo)
{
      if (_decompo.invalid())
       {
        ...
        std::cout << _decompo.fitness() << std::endl;

}

void PlanningEvalMO::adaptive_search_strategy()
{
...
double PlanningEvalMO::additive_cost(  PlanningMO & _decompo)
{
...
double PlanningEvalMO::max_cost(  PlanningMO & _decompo)
{
 ...
 void PlanningEvalMO::call (PlanningMO & _decompo){
	
	double fit;
	
	fit = makespan(_decompo);
	
	_decompo.fitness(fit);
 ...
 double PlanningEvalMO::makespan(PlanningMO &  decompo)
{
    double _makespan; 
...
void PlanningEvalMO::post_call(PlanningMO & decompo ) {
        decompo.plan().search_steps( decompo.get_number_evaluated_nodes() );
        
        } 
        
        
 unsigned int PlanningEvalMO::solve_next(PlanningMO & decompo, Fluent** next_state, unsigned int next_state_nb, long max_evaluated_nodes ) {
  ...
  void PlanningEvalMO::compress(PlanningMO & decompo ) {
  					 #ifndef NDEBUG
 ...
 void  PlanningEvalMO::free_yahsp_structures(){
 {
                                 #ifndef NDEBUG
