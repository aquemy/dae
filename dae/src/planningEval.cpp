...
void PlanningEval::weighted_search_strategy(Planning & _decompo)
{
  
  unsigned what = rng.roulette_wheel(rates);
  
  if (what == 0){ yahsp_set_optimize_length(); }//std::cout<<" yahsp_set_optimize_length()"<<std::endl;}
  else	if (what ==1) {yahsp_set_optimize_cost(); }// std::cout<<"yahsp_set_optimize_cost()"<<std::endl;}
	 else if (what ==2) {yahsp_set_optimize_makespan_max(); }// std::cout<<"yahsp_set_optimize_makespan_max()"<<std::endl;}
	      else {yahsp_set_optimize_makespan_add(); }// std::cout<<"yahsp_set_optimize_makespan_add()"<<std::endl;}
	      
	      
 if (rand_seed)
   //yahsp_set_seed(rng.rand());
    yahsp_set_seed(_decompo.indivSeed());
